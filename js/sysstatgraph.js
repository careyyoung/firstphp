var sysstatgraph = function() {
	...
	return {
		init: function() {
			...
			var graphwidth = getviewportwidth() - 400,
				graphheight = 200,
				timepointlist = sysstatgraph.statdata.timepointlist;
			...
			// graph3.addgraphline('cpu0_User',getvaluelist('cpu_0_user'));
			graph3.addgraphline('cpu1_User',getvaluelist('cpu_1_user'));
			// graph3.addgraphline('cpu2_User',getvaluelist('cpu_2_user'));
			// graph3.addgraphline('cpu3_User',getvaluelist('cpu_3_user'));
			...
			// - percentage of CPU utilisation that occurred while executing at the system level
			// - Percentage of time that the CPU or CPUs were idle during which the system had an outstanding disk I/O request
			var graph31 = new rendergraph.instance(
				'CPU-N utilisation （）',
				graphwidth,graphheight,
				starttimestamp,timepointlist
			);
			graph31.setpercent();
			// graph31.addgraphline('User',getvaluelist('cpuuser'));
			graph31.addgraphline('cpu1_User',getvaluelist('cpu_0_user'));
			graph31.addgraphline('cpu2_User',getvaluelist('cpu_1_user'));
			graph31.addgraphline('cpu3_User',getvaluelist('cpu_2_user'));
			graph31.addgraphline('cpu4_User',getvaluelist('cpu_3_user'));
			// graph31.addgraphline('System',getvaluelist('cpusystem'));
			// graph31.addgraphline('IOwait',getvaluelist('cpuiowait'));
			graph31.render();
			...
			var graph6 = new rendergraph.instance(
				'System load averages （Load Average < CPU个数 * 核数 *0.7 = 6*2*0.7 = 8.4）',
				graphwidth,graphheight,
				starttimestamp,timepointlist
			);
