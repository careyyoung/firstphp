...
class importstatfiledata {
	...
	private function getfiledatetimestamp($inputfirstline) {
        ...
        // 获取sar文件第一行中的日期，兼容多种日期格式：YYYY-MM-DD、MM/DD/YYYY、MM/DD/YY……
        //return (preg_match('/(?P<year>\d{4})-(?P<month>\d{2})-(?P<day>\d{2})/',rtrim($inputfirstline),$match))
        return (preg_match('/(?P<month>\d{2})\/(?P<day>\d{2})\/(?P<year>\d{4})/',rtrim($inputfirstline),$match))
			? mktime(0,0,0,intval($match['month']),intval($match['day']),intval($match['year']))
			: (preg_match('/(?P<year>\d{4})-(?P<month>\d{2})-(?P<day>\d{2})/',rtrim($inputfirstline),$match))
                ? mktime(0,0,0,intval($match['month']),intval($match['day']),intval($match['year']))
                : (preg_match('/(?P<month>\d{2})\/(?P<day>\d{2})\/(?P<year>\d{2})/',rtrim($inputfirstline),$match))
                    ? mktime(0,0,0,intval($match['month']),intval($match['day']),intval($match['year']))
                    : FALSE
            ;
	}
...
//            现在改成读取全部cpu数据
            if ($inputlinepartlist[1] == 'all'){
                ...
                $this->recordstat($inputtimestamp,'cpu_'.$inputlinepartlist[1].'_user',floatval($inputlinepartlist[2]));
            }
			return;
		}
